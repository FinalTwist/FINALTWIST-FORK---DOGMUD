...
- After every task: `gofmt -l internal/ modules/ .` prints nothing (compare against master for any file you think is pre-existing: the rename leaves imports unsorted), `go build ./...`, `go vet ./...`, `go test ./... -count=1` pass, AND `DOGMUD_BOOT_SMOKE=1 go test . -count=1` (CI sets it; it holds a key list naming Go types). Templates that read Go names live in `_datafiles/world/*/templates` AND `_datafiles/html/admin/**` (found in review of Tasks 2 and 3). The Task 0 freeze tests are green at the end of every task; a red freeze test is a slice 2 regression, fixed before continuing.
