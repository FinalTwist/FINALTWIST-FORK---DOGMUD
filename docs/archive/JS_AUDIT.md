...
| Conditions | ~68 | ~1,500 | Trivial → Medium |
...
actual mechanical effects (damage, healing, condition application, hit/miss)
...
### 2.2 Conditions (68 files)

Conditions use lifecycle hooks: `onStart` → `onTrigger` (per tick) → `onEnd`.

**Flavor-only conditions (~40 files, 9-12 lines):** Send a message on start
and end. `onTrigger` is empty. All mechanical effects (stat mods, flags,
duration) live in the condition YAML definition and Go backend.

**Healing/damage-over-time conditions (~15 files, 14-25 lines):** Calculate a
...
**Utility conditions (~5 files):** Minor antidote removes poison conditions.
Death recovery rolls dice for resource restoration. Tutorial conditions
gate command access.
...
hit-and-run boss tactics with sneak/hide condition management, and timed
...
Mostly simple `onCommand_use` handlers: grant conditions, spawn items (newbie
...
- **VM model:** One VM per unique script definition, cached in type-specific
  maps (`roomVMCache`, `mobVMCache`, `conditionVMCache`, `spellVMCache`,
  `itemVMCache`)
...
- Per-type files (`condition.go`, `spell.go`, `item.go`) — 200-250 lines each
...
- **~40 flavor-only condition scripts** follow the same 9-12 line template
...
test a spell or condition script in isolation.
...
conditions, mobs, rooms, and items would reference a Go function name in
...
For spells/conditions that need logic (summoning, teleportation, charm),
...
and hot-reload matter most. Move spells, conditions, and simple room/item
...
| Flavor-only conditions | ~40 | YAML text fields | Low — mechanical migration |
| Stub room scripts | ~30 | Delete (no-ops) | Trivial |
| Healing/DoT conditions | ~15 | Go functions or YAML config | Low-Medium |
...
we could slim it significantly — remove spell/condition/item VM management
...
**Flavor-only spells and conditions (~75 files):**
...
- **Cost:** Requires adding `cast_text`/`wait_text`/`magic_text` fields
  ...
  for condition `start_text`/`trigger_text`/`end_text`.
- **Risk:** Low. These scripts have zero logic.
- **Upstream impact:** Moderate — changes spell/condition loading in Go engine.
...
**Healing/DoT conditions (15 files):**
- **Benefit:** Replace percentage-based tick logic with YAML config fields
  (`heal_pct: 0.08`, `damage_pct: 0.05`). The Go condition tick handler
  already exists — just needs to read these fields.
- **Cost:** Low. Add fields to condition schema, update tick handler.
...
- **Benefit:** Most are trivial (use → give condition / spawn item). Could be
  YAML fields (`on_use_condition: 15`, `on_use_spawn_item: [4, 6, 100]`).
...
| Flavor condition text → YAML | ~40 | ~400 | Low | **High** |
...
| `condition.go` — 237 lines | **Delete** (no more condition JS) |
...
1. **Hot-reload loss for spells/conditions:** Moving flavor text to YAML means
...
2. **Upstream merge friction:** Any changes to the spell/condition loading
...
- Add `start_text`, `trigger_text`, `end_text` fields to condition YAML schema
- Go spell/condition resolution sends these texts when present, skips JS
...
- Write parameterized `HealOverTime()` / `DamageOverTime()` for condition ticks
- Replace 15 healing/DoT condition scripts with YAML config fields
...
- Remove unused bridge code (`spell.go`, `condition.go`, `item.go`)
...
### Condition Scripts (dogmud) — 28 files
```
_datafiles/world/dogmud/conditions/
