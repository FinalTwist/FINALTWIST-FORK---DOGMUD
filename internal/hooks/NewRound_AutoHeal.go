...
func AutoHeal(e events.Event) events.ListenerReturn {
	...
	for _, userId := range onlineIds {
		...
		if user.Character.Health > 0 {
			...
			if !inCombat {
				...
				if regenMult := user.Character.Buffs.Effect(conditions.EffectRegenMult); regenMult > 1.0 {
				...
				if user.Character.Buffs.HasEffect(conditions.EffectRegenMult) {
				...
				if user.Character.Buffs.HasEffect(conditions.EffectRegenMult) {
					regenMult := user.Character.Buffs.Effect(conditions.EffectRegenMult)
	...
	for _, mobInstId := range mobs.GetAllMobInstanceIds() {
		...
		if !mobInCombat {
			...
			if regenMult := mob.Character.Buffs.Effect(conditions.EffectRegenMult); regenMult > 1.0 {
				hpRegen *= regenMult
			}
			...
			if mob.Character.Buffs.HasEffect(conditions.EffectRegenMult) {
				regenMult := mob.Character.Buffs.Effect(conditions.EffectRegenMult)
