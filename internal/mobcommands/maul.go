...
func Maul(rest string, mob *mobs.Mob, room *rooms.Room) (bool, error) {
	...
	if result.Hit {
		sendMoveEvent("maul", "hit", ids, aud, messaging.CategoryHitNaturalSharp, damageTokens)
	} else if result.Damage > 0 {
		// Defended-partial: the actee line still carries the damage from the
		// store; the room line names the defence that blunted the maul, so it
		// is swapped for the defence triad's ToRoom text when a defence
		// actually fired.
		roles, _ := renderMoveEvent("maul", "partial", ids, damageTokens)
		...
		sendMoveEvent("maul", "miss", ids, aud, messaging.CategoryHitNaturalSharp, nil)
	}
