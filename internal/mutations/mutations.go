...
// GetOnHitConditions returns the buff ids that owned mutations apply to a struck
...
// GetReflectRiderConditions returns the buff ids that owned mutations apply to an
...
func GetReflectRiderConditions(owned map[string]int) []int {
	var out []int
	for id := range owned {
		spec := GetMutation(id)
		if spec == nil {
