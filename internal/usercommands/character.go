...
func cmdCharacterHire(user *users.UserRecord, room *rooms.Room, cmdPrompt *prompt.Prompt, altNames []string, nameToAlt map[string]characters.Character, hiredOutChars map[string]characters.Character) (bool, error) {
	...
	if match != `` {
		...
		_ = m.Character.AddCondition(99, true) // Give a perma-gear condition, so that items can't be removed.
		// NOTE: dogmud renumbered the default condition set — the upstream "Alt
		// Character Mob" perma-gear condition was id 36, which here is psychic_anchor.
		// The perma-gear condition lives at id 99 (99-alt_character_mob.yaml).
