...
func init() {
	...
	// State-touching inbound Char.* ops are handled on MainWorker too
...
func (g *GMCPModule) HandleIAC(connectionId uint64, iacCmd []byte) bool {
	...
	if len(iacCmd) >= 5 && iacCmd[len(iacCmd)-2] == term.TELNET_IAC && iacCmd[len(iacCmd)-1] == term.TELNET_SE {
		...
		// and — via rooms.LoadRoom in Char.Action.Try — the room manager's
		// plain maps). HandleIAC runs on the per-connection goroutine, so
		// doing that work here races the world tick (concurrent map access ->
		// uncatchable fatal error). Defer to MainWorker via an event, exactly
		// as Build.* does below; handleCharOp holds the real logic. Copy the
		// payload — it aliases the IAC read buffer, which is reused after
		// HandleIAC returns.
		case `Char.Automation.Set`, `Char.Automation.Remove`,
			`Char.Action.Try`, `Char.Quests.Focus`:
			events.AddToQueue(GMCPCharOp{
				ConnectionId: connectionId,
				Command:      command,
