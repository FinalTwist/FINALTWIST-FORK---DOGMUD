...
func buildMessages(in promptInput) []chatMessage {
	...
	sys.WriteString("- You know only the places you have been. find_place (with a query) makes you think back to where something was. go_to a place [ref] (r123) walks you there by the way you know; go_to \"owner\" walks you back. explore (ref: an unexplored way out of here) takes one step into the unknown. You only set off on your own from beside " + owner + " while they stay put, and you come back afterwards; if they move on you are called back. You cannot fight on command: your body fights for itself.\n")
...
func formatStimulus(s stimulus, ownerName string) string {
	...
	case `arrived`:
		if s.Authorized {
			return fmt.Sprintf(`You have reached %s, on the errand %s asked of you.`, s.Text, ownerName)
		}
		return fmt.Sprintf(`You have reached %s.`, s.Text)
	case `trip`:
		return s.Text
	case `fight`:
...
func tripWords(p *travelPlan) string {
	...
	return fmt.Sprintf(`You are on your way to %s (%d of %d steps taken).`, p.DestName, p.Next, len(p.Steps))
}
