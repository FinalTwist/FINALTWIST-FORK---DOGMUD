...
func moderate(baseURL string, apiKey string, model string, timeout time.Duration, texts []string) []bool {
	if len(texts) == 0 {
		return nil
	}
	...
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&mr); err != nil || len(mr.Results) != len(texts) {
		return nil
	}
...
func moderateDecision(d *Decision, baseURL string, apiKey string, model string, timeout time.Duration) int {
	var texts []string
	...
	flags := moderate(baseURL, apiKey, model, timeout, texts)
	if flags == nil {
		return 0
	}
	removed := 0
