...
func buildConfig(get getter) Config {
	...
	c := Config{
		...
		DeepModel:                    strings.TrimSpace(asString(get(`DeepModel`))),
		...
		DailyTokensPerCompanion:      asInt(get(`DailyTokensPerCompanion`), 0),
		ModerateOutput:               asBool(get(`ModerateOutput`)),
