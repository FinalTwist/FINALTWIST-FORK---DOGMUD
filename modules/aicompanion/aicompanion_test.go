...
func TestBuildConfigDefaultsAndFloors(t *testing.T) {
	c := buildConfig(nil)
	if !c.Enabled {
		t.Fatal("the module is on unless switched off")
	}
...
func TestCombatBasics(t *testing.T) {
	...
	if healthPct(25, 100) != 25 || healthPct(5, 0) != 100 {
		t.Fatal("health percent")
	}
	m := &AICompanionModule{}
...
func TestModelTierRouting(t *testing.T) {
	...
	if tierFor([]stimulus{{Kind: `noticed`}, {Kind: `idle`}}) != tierFast {
...
func TestModerationRemovesFlaggedLines(t *testing.T) {
	...
	removed := moderateDecision(&d, srv.URL, `key`, `omni-moderation-latest`, time.Second)
	if removed != 2 || len(d.Speech) != 1 || d.Speech[0].Text != `hello` || d.Action.Verb != `none` {
		t.Fatalf("moderation: removed=%d decision=%+v", removed, d)
	}
	// An unreachable endpoint must not silence the companion.
	d2 := Decision{Speech: []SpeechLine{{Kind: `say`, Text: `hello`}}}
	if moderateDecision(&d2, `http://127.0.0.1:1`, `key`, `m`, 200*time.Millisecond) != 0 || len(d2.Speech) != 1 {
		t.Fatal("moderation failure must keep the line")
	}
}
...
func TestMeetingDefaultsAndLeaving(t *testing.T) {
	c := buildConfig(nil)
	if !c.AutoBond || !c.AutoBondExisting || c.AutoBondProfile != `mara` || len(c.MeetSkipZones) != 1 {
...
func TestTokenReservationSettles(t *testing.T) {
	...
	// The whole worst case is held, not one completion's worth.
	if got := worstCaseTokens(1000, 500, 2, true); got != (1000+500)*3*2 {
		t.Fatalf("worst case = %d", got)
	}
}
