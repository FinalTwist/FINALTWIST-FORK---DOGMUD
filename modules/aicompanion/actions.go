...
// ownerPrompted reports whether this decision came from the owner at all,
// rather than from a stranger's talk.
...
func (m *AICompanionModule) performAction(c *controller, mob *mobs.Mob, owner *users.UserRecord, sc *scene,
	...
	case `go_to`:
		return m.goTo(c, mob, owner, a.Ref, stims)
	case `explore`:
	...
	case `give`, `show`:
		...
		if a.Verb == `give` && a.To != `owner` && a.To != `` && !canPartWithItem(c.mind, mob, &t.Item) {
			return actionOutcome{Refused: `you will not give that away`}
		}
...
func (m *AICompanionModule) lookAt(c *controller, mob *mobs.Mob, room *rooms.Room, t *thing, delay float64, visible bool) actionOutcome {
	...
	case `npc`:
		if tm := mobs.GetInstance(t.MobInstanceId); tm != nil {
			...
			desc = plainText(d) + ` They look ` + healthWords(tm.Character.Health, tm.Character.HealthMax.Value) + `.`
		}
		emote = fmt.Sprintf(`studies %s for a moment.`, t.Name)
	case `player`:
		if u := users.GetByUserId(t.UserId); u != nil && u.Character != nil {
			desc = plainText(u.Character.Description) + ` They look ` + healthWords(u.Character.Health, u.Character.HealthMax.Value) + `.`
		}
		emote = fmt.Sprintf(`studies %s for a moment.`, t.Name)
	case `corpse`:
...
func (m *AICompanionModule) goTo(c *controller, mob *mobs.Mob, owner *users.UserRecord, ref string, stims []stimulus) actionOutcome {
	if ref == `owner` {
	...
	c.mind.addLine(Line{Kind: `event`, Text: `You set off toward ` + c.travel.DestName + `.`}, m.cfg.WorkingMemoryLines)
	return actionOutcome{Issued: true}
}
