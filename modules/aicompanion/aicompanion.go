...
func init() {
	...
	// The config overlay has to be mounted here: it is where Enabled itself
	// is read from. The help pages come later, in registerCommands, with
	// the commands they describe.
	if err := module.plug.AttachFileSystem(configFiles); err != nil {
		panic(err)
	}
