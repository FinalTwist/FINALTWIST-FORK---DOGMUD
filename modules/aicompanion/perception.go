...
func healthWords(cur int, max int) string {
	if max <= 0 {
		return `unhurt`
	}
	pct := cur * 100 / max
...
func describeSituation(mob *mobs.Mob, owner *users.UserRecord) string {
	...
	if owner != nil && owner.Character != nil {
		...
		if ownerHere && !ownerVisible {
			...
			fmt.Fprintf(&b, "%s is here with you (%s)%s.\n", owner.Character.Name,
				healthWords(owner.Character.Health, owner.Character.HealthMax.Value), ownerLooks(owner))
		} else {
	...
	// Yourself.
	self := healthWords(mob.Character.Health, mob.Character.HealthMax.Value)
