	...
	for _, u := range users.GetAllActiveUsers() {
		...
		if c == nil || c.profile.MobId != p.MobId {
			...
			if mob := mobs.GetInstance(comp.InstanceId); mob != nil && now.Unix() < c.mind.FallenUntilUnix {
				if half := mob.Character.HealthMax.Value / 2; half > 0 && mob.Character.Health > half {
					mob.Character.Health = half
				}
...
func (m *AICompanionModule) dispatch(c *controller) {
	...
	go func() {
		...
		defer func() {
			...
			// However this ended, the tokens held for it must go back, or
			// the day's budget drains on calls that never happened.
			if !applied {
				util.LockMud()
		...
		if res.Err == nil {
			...
			if err != nil {
				...
				if moderation {
					res.Moderated = moderateDecision(&d, call.BaseURL, call.APIKey, moderationModel, 5*time.Second)
				}
				res.Parsed = &d
			}
		}
		...
		// unlock: if applying the result panics, the game loop is still
		// released, and the cleanup below can take the lock safely.
		func() {
			util.LockMud()
