...
var (
	...
	// combatMoves are the engine's special moves a two-armed, two-legged
	// companion can use. The engine decides whether one would land right
	// now (body, gear, cooldown); skills decide how well it goes.
...
var actionVerbs = []string{
	...
	`browse`, `buy`, `sell`, `loot`, `take_from`, `craft`,
}
	...
	return object(
		...
		map[string]any{
			...
			`action`: object([]string{`verb`, `ref`, `to`, `query`}, map[string]any{
				...
				`ref`:   str(`The [ref] the action is about: a thing (t2, p1, w1), a place (r123), a recipe you could make here (k1), "owner" for go_to, or an exit name for explore. Empty for none, forage, search and find_place.`),
				`to`:    str(`For give and show: the [ref] of who receives it, or "owner". For put: the [ref] of the container. Otherwise empty.`),
				`query`: str(`For find_place: what you are trying to remember the way to. For sayto: the words you say. For buy: how many (a number). For take_from: which item in the container. Otherwise empty.`),
			}),
