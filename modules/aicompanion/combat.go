...
func healthPct(cur int, max int) int {
	if max <= 0 {
		return 100
	}
	return cur * 100 / max
...
func fightLines(f *fightState, room *rooms.Room, mob *mobs.Mob, owner *users.UserRecord) []string {
	...
	for _, id := range ids {
		...
		out = append(out, fmt.Sprintf(`[%s] %s: %s, fighting %s; %s%s`, ref, m.Character.Name,
			healthWords(m.Character.Health, m.Character.HealthMax.Value), who, odds, mark))
	}
	...
	for _, id := range uids {
		...
		out = append(out, fmt.Sprintf(`%s (a person) is in this fight: %s. You cannot choose a person as your target.`,
			u.Character.Name, healthWords(u.Character.Health, u.Character.HealthMax.Value)))
	}
	out = append(out, fmt.Sprintf(`You are %s. %s is %s.`, healthWords(mob.Character.Health, mob.Character.HealthMax.Value),
...
func ownerHealth(owner *users.UserRecord) string {
	...
	return healthWords(owner.Character.Health, owner.Character.HealthMax.Value)
}
...
func (m *AICompanionModule) combatTick(c *controller, u *users.UserRecord, round uint64) {
	...
	selfPct := healthPct(mob.Character.Health, mob.Character.HealthMax.Value)
	ownerPct := 100
	ownerHere := u != nil && u.Character != nil && u.Character.RoomId == mob.Character.RoomId
	if ownerHere {
		ownerPct = healthPct(u.Character.Health, u.Character.HealthMax.Value)
	}
	...
	selfBand := healthWords(mob.Character.Health, mob.Character.HealthMax.Value)
	ownerBand := ``
	if ownerHere {
		ownerBand = healthWords(u.Character.Health, u.Character.HealthMax.Value)
	}
